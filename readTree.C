...
const float pt_trig_up = 12.5;
const float pt_trig_lo = 6.5;             
const float pt_asso_up = 6.5;
...
TFile fout("weiLi_500GeV_processed.root","RECREATE");
...
TH1D *hdPhiRawce = new TH1D("hdPhiRawce","hdPhiRawce",Phibin, -PI,PI);
TH1D *hdPhiRawbe = new TH1D("hdPhiRawbe","hdPhiRawbe",Phibin, -PI,PI);
TH1D *hdPhiRawbce = new TH1D("hdPhiRawbce","hdPhiRawbce",Phibin, -PI,PI);
...
void analyze()
{
  ...
  int nfile = 0;
  nfile += chain->Add("liweiTemplate_part1.root");
  ...
  for(int i = 0; i < nentries; i++){
	...
	if(assoh_eta > 1. || assoh_eta < -1.) continue;
